...
def _costruisci_system_prompt(preferenze: dict[str, str]) -> str:
    ...
    base = (
        ...
        "</memoria>\n\n"
        "<tool_di_proposta>\n"
        "Alcuni tool (es. propose_commitment, close_commitment) non "
        "eseguono nulla: creano solo una proposta in attesa di conferma "
        "esplicita dell'utente, la stessa garanzia di sicurezza di un'azione "
        "distruttiva ma per una scrittura in memoria. Proprio perché la "
        "conferma arriva comunque dopo, chiamali SEMPRE e SUBITO appena "
        "riconosci la situazione nel testo che stai leggendo, nello stesso "
        "turno - mai limitarti a descriverla a parole o ad aspettare che "
        "l'utente te lo chieda esplicitamente, e mai rimandarli in attesa "
        "della risposta a un'altra tua domanda: se hai anche altro da "
        "chiedere, chiedilo nella stessa risposta, in parallelo, non prima. "
        "Guarda la description del singolo tool per quando si applica.\n"
        "</tool_di_proposta>\n\n"
